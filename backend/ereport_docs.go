	...
	writeJSON(w, http.StatusOK, map[string]any{
		...
		"payloadSchema": map[string]any{
			...
			"rootFields": map[string]string{
				...
				"validationCriteria": "string[] of criteria labels (report-level labels; optional)",
				"sections":           "sections[].groups[].items[] plus optional sections[].items[] open issues",
			},
			"sectionFields": map[string]string{
				...
				"items":          "open issues at section level (same item shape)",
				"groups":         "subsections / features",
			},
			"groupFields": map[string]string{
				"id":             "stable group id",
				"title":          "subsection / feature title",
				"productHistory": "free-text product / concept history for the group",
				"items":          "issues under this group",
			},
			"itemFields": map[string]string{
				"id":               "stable item id",
				"nombre":           "short issue name",
				"status":           "aprobado|reprobado|no_aplica|empty — synced from checklist in the UI",
				"incidencia":       "issue text",
				"solucion":         "resolution / action text",
				"fechaIncidencia":  "YYYY-MM-DD or datetime-local",
				"fechaSolucion":    "YYYY-MM-DD or datetime-local",
				"checklist":        "{id,label,checked}[] UX tests (see checklist)",
				"criteriaStatus":   "optional map of validationCriteria label → status",
				"imagesIncidencia": "{id,mime,name,url} file refs (legacy dataUrl left untouched on import)",
				"imagesSolucion":   "{id,mime,name,url} file refs",
			},
		},
	})
}
